...
- **Real on-chain settlement.** Sessions in `PAYMENT_MODE=circle` produce `settlement_batch` ledger rows with `gatewayStatus: "settled"` and a real Gateway tx ref (a recent one: `9eafa916-...`, $0.0048, payer -> seller), carrying the signed EIP-3009 authorization as the payment proof. Earlier settles (`b81b1a28`, `b3d8de1c`) decremented the buyer's on-chain Gateway balance, confirming the batched settlement lands on-chain.
- **Batch ref vs EVM hash.** Because Gateway settles asynchronously in batches, the ref returned at settle time is a Circle batch id (a UUID), and the EVM tx hash posts when the batch flushes on-chain. The app only renders an explorer `/tx/` link once a ref is a real `0x` hash; until then it shows the batch ref and links the payer/recipient address (which resolves on Arcscan) as the verify path.
